...
def get_args():
    ...
    parser.add_argument('--save_coms',
                        dest='save_coms',
                        required=False,
                        default=False,
                        help='prefix of file to save random communities to, if blank they are not saved (faster)')
...
nodes = list(G.nodes)
hpo_nodes = [x for x in G.nodes if 'HP:' in x]
gene_nodes = [x for x in G.nodes if 'HP:' not in x]
outfile = None
# for each com
for i, line in enumerate(open(coms, 'r')):
    ...
    if outfile is not None:
        outfile.close()
    if args.save_coms:
        outfile = open(args.save_coms + str(i) + '.txt', 'w')
    ...
    num_hpos = sum('HP:' in x for x in com)
    num_genes = len(com) - num_hpos
    for j in range(reps):
        # choose a random sample with equal number of HPO and genes as the original com
        random_hpos = random.sample(hpo_nodes, num_hpos)
        random_genes = random.sample(gene_nodes, num_genes)
        random_sample = random_hpos + random_genes

        if outfile is not None:
            outfile.write('\t'.join([str(j)] + random_sample) + '\n')
